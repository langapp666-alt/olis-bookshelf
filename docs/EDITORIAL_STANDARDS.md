...
7. **Affiliate links use the live site tag.** `affiliateTag` / `PUBLIC_AFFILIATE_TAG` in `src/lib/site.ts`. Do not invent a second tag. Each title in an order table gets its own Amazon URL via `amazonBookUrl()` — **search by default**; an ASIN deep link only when a verified ASIN is already in content data. Never invent ASINs. See `HOW_IT_EARNS.md`.
