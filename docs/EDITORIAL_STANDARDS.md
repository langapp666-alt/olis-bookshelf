...
7. **Affiliate links use the live site tag.** `affiliateTag` / `PUBLIC_AFFILIATE_TAG` in `src/lib/site.ts`. Do not invent a second tag. Each title in an order table gets its own Amazon URL via `amazonBookUrl()` — currently tagged title + author search only. Do not emit `/dp/{id}` unless the id is a validated amazon.com ASIN (Open Library ISBN-10 is not enough). See `HOW_IT_EARNS.md` and `ASIN_DEEP_LINKS.md`.
