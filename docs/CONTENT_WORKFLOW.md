...
9. Per-title Amazon URLs via `OrderTable` / `amazonBookUrl()` (site tag; search by default, ASIN only if verified in data). Do not invent ASINs. Buy-intent question hubs may set optional `buy:` frontmatter for one book-1 Amazon control. Do not add affiliate-explainer prose to the guide; sitewide disclosure lives in the footer and on `/disclosure/` only.
