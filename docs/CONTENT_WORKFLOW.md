...
9. Per-title Amazon URLs via `OrderTable` / `amazonBookUrl()` (site tag; ASIN deep link when on file, else title + author search). Do not invent ASINs. Do not add affiliate-explainer prose to the guide; sitewide disclosure lives in the footer and on `/disclosure/` only.
