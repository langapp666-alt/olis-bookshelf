...
Amazon Associates is **active**. Per-title shop links use `amazonBookUrl()` → tagged title + author search (`tag=olisbookshelf-20`). ASIN `/dp/` deep links are **off** (`ASIN_DEEP_LINKS_ENABLED = false`): Open Library ISBN-10 is not a reliable amazon.com ASIN. See `docs/ASIN_DEEP_LINKS.md`. Do not invent ASINs.
...
Short **question hubs** live at `/questions/` (for example `/questions/where-to-start-the-cosmere/`). They answer one search intent, then send the reader to the matching guide. They are not a second catalog of affiliate pages. Later batches cover Witcher, Dark Tower, Vorkosigan, Malazan, First Law, Expanse, Red Rising, Tolkien, Narnia, Mistborn eras, Thursday Murder Club, Rivers of London, Bobiverse, Roots of Chaos, Broken Kingdoms, Revelation Space, Foundation, Poppy War, Artemis Fowl, Temeraire, Marie Lu’s Legend (not Gemmell), Silo/Wool, ACOTAR where-to-start, and Dune where-to-start.
...
`src/lib/site.ts` exports `affiliateTag` from `PUBLIC_AFFILIATE_TAG` (default `olisbookshelf-20`). Shop CTAs use tagged title + author search. Do not re-enable `/dp/{ASIN}` until each id is verified on amazon.com (see `docs/ASIN_DEEP_LINKS.md`). Do not invent ASINs. Keep `/disclosure/` accurate if the tag changes.
