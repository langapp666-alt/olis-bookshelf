...
export function matchGuides(
  ...
  for (const entry of index) {
    const fields = [entry.series, entry.title, entry.author, entry.slug];
    let score = 0;
    ...
    const blob = `${entry.series} ${entry.author} ${entry.title} ${entry.slug}`.toLowerCase();
...
export function pickRandomGuide(
  ...
  const pool = exceptSlug ? index.filter((entry) => entry.slug !== exceptSlug) : [...index];
